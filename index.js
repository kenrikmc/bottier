...
// Danh sách ID các ROLE được phép nhập kết quả
...
client.once('ready', () => {
    console.log(`✅ Bot online! Chế độ kiểm tra ROLE đã sẵn sàng.`);
});
...
client.on('messageCreate', async (message) => {
    if (message.content === '!setup') {
        ...
        await message.channel.send({ content: 'Nhấn nút để bắt đầu test:', components: [row] });
    }
});
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isButton() && interaction.customId === 'open_ticket') {
        ...
        await ticket.send({ content: `Chào ${interaction.user}, Tester có Role phù hợp hãy nhấn nút bên dưới.`, components: [row] });
        await interaction.reply({ content: `Đã mở ticket: ${ticket}`, ephemeral: true });
    }

    // 2. Kiểm tra Role và hiện Form
    if (interaction.isButton() && interaction.customId === 'go_form') {
        
        // LỆNH KIỂM TRA: Xem người bấm có sở hữu bất kỳ Role nào trong danh sách không
        const hasPermission = interaction.member.roles.cache.some(role => ALLOWED_ROLE_IDS.includes(role.id));

        if (!hasPermission) {
            return interaction.reply({ 
                content: '❌ Bạn không có Role Tester để nhập kết quả!', 
                ephemeral: true 
            });
        }
        ...
        modal.addComponents(
            new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('f_name').setLabel("Tên game (Ingame)").setStyle(TextInputStyle.Short)),
            new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('f_mode').setLabel("Mode test").setStyle(TextInputStyle.Short)),
    ...
    if (interaction.isModalSubmit() && interaction.customId === 'modal_result') {
        const nameGame = interaction.fields.getTextInputValue('f_name');
        ...
        if (logChannel) {
            ...
            await logChannel.send({ content: `<@${targetUserId}>\n**Kết Quả Test của ${nameGame}**`, embeds: [embed] });
        }

        await interaction.reply({ content: '✅ Đã gửi kết quả!' });
